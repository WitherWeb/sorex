...
document.addEventListener("DOMContentLoaded", function () {
  ...
  menuToggle.addEventListener("click", function () {
    if (menu.style.height && menu.style.height !== "0px") {
      // Collapse the menu
      menu.style.height = menu.scrollHeight + "px"; // Set to current height to trigger transition
      setTimeout(() => {
        menu.style.height = "0"; // Collapse to zero height
      }, 10);
    } else {
      // Expand the menu
      menu.style.height = menu.scrollHeight + "px"; // Set to full height to trigger transition
      setTimeout(() => {
        menu.style.height = "none"; // Remove max-height after transition for flexibility
      }, 500); // Match this with the transition duration in CSS
    }
  });

  const swiper = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](".product-list__swiper", {
    slidesPerView: 4.3,
    // Показывать 1.5 слайда одновременно
    spaceBetween: 20,
    ...
    breakpoints: {
      1440: {
        slidesPerView: 4.3,
        // Показывать 3 слайда при ширине экрана 1024px и больше
        spaceBetween: 20 //
      },
  ...
  if (document.querySelector(".show-more-text-btn")) {
    const showMoreButtons = document.querySelectorAll(".show-more-text-btn");
    showMoreButtons.forEach(function (button) {
      button.addEventListener("click", function () {
        const textContent = button.closest(".text-container").querySelector(".text-content");
        textContent.classList.toggle("text-content--expanded");
        button.style.display = "none";
      });
    });
  }
  if (document.querySelector(".footer__nav-drop-button")) {
    const showMoreButtons = document.querySelectorAll(".footer__nav-drop-button");
    showMoreButtons.forEach(function (button) {
      button.addEventListener("click", function () {
        const footerList = button.closest(".footer__nav-column");
        footerList.classList.toggle("footer__nav-column--expanded");
        button.innerHTML = "-";
      });
    });
  }
